...
from app.job_sources import fetch_remotive_jobs
from app.storage import JobStorage
from scripts.source_scan_utils import ingest_source_jobs


def run_remotive_scan(storage: JobStorage | None = None) -> dict:
    storage = storage or JobStorage(DEFAULT_GMAIL_DB_PATH)
    return ingest_source_jobs(fetch_remotive_jobs(), storage)


def main() -> None:
    summary = run_remotive_scan()
    print("Remotive scan complete")
